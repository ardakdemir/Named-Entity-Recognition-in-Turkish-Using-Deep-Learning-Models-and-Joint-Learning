...
## may use in future
class NerEntry:
    def __init__(self,id,form,nertag):
        self.id=id
        self.form=form
        self.capInfo=0
        if form[0].isupper():
...
## ilerde belki lazim olur
def ner_vocab(file_path):
    wordsCount = Counter()
    nerCount = Counter()
    capCount = Counter()
...
def vocab_ner(conll_path):
    ...
    root = ConllEntry(0, '*root*', '*root*', 'O', 'ROOT-CPOS', '_', -1, 'rroot', '_', '_')
    root.idChars = [1, 2]
    tokens = [root]
    ...
    for line in open(conll_path, 'r'):
        ...
        if not tok or line.strip() == '':
            if len(tokens) > 1:
                ...
                capCount.update([node.capInfo for node in tokens if isinstance(node, ConllEntry)])
            tokens = [root]
        else:
            if line[0] == '#' or '-' in tok[0] or '.' in tok[0]:
                tokens.append(line.strip())
            else:
                entry = ConllEntry(int(tok[0]), tok[1], tok[2], tok[-1].split()[-3].replace("\"","").replace(",",""), tok[4], tok[5],
                                   int(tok[6]) if tok[6] != '_' else -1, tok[7], tok[8], '-')
    ...
    if len(tokens) > 1:
        ...
        relCount.update([node.relation for node in tokens if isinstance(node, ConllEntry)])
        capCount.update([node.capInfo for node in tokens if isinstance(node, ConllEntry)])
    return (wordsCount, {w: i for i, w in enumerate(wordsCount.keys())}, c2i, posCount.keys(), relCount.keys(),capCount.keys())
...
def read_conll_predict_ner(fh, c2i, wordsCount):
    # Character vocabulary
    root = ConllEntry(0, '*root*', '*root*', 'O', 'ROOT-CPOS', '_', -1, 'rroot', '_', '_')
    root.idChars = [1, 2]
    tokens = [root]
